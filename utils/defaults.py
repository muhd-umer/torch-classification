...
def get_network_G_config(network_G, scale):
    ...
    if kind_G in ("rrdb_net", "esrgan", "evsrgan", "esrgan-lite"):
        ...
        full_network_G["norm_type"] = network_G.pop(
            "norm_type", None
        )  # "instance" normalization, "batch" normalization or no norm
        full_network_G["mode"] = network_G.pop(
            "mode", "CNA"
        )  # CNA: conv->norm->act, NAC: norm->act->conv
        ...
        full_network_G["nr"] = network_G.pop(
            "nr", 3
        )  #  number of residual layers in each RRDB block
        full_network_G["in_nc"] = network_G.pop(
            "in_nc", 3
        )  # num. of input image channels: 3 for RGB and 1 for grayscale
        full_network_G["out_nc"] = network_G.pop(
            "out_nc", 3
        )  # num. of output image channels: 3 for RGB and 1 for grayscale
        full_network_G["gc"] = network_G.pop("gc", 32)  #
        if kind_G == "evsrgan":
            full_network_G["convtype"] = network_G.pop(
                "convtype", "Conv3D"
            )  # Conv3D for video
        else:
            full_network_G["convtype"] = network_G.pop(
                "convtype", "Conv2D"
            )  # Conv2D | PartialConv2D | DeformConv2D | Conv3D
        full_network_G["act_type"] = network_G.pop("net_act", None) or network_G.pop(
            "act_type", "leakyrelu"
        )  # swish | leakyrelu
        full_network_G["gaussian_noise"] = network_G.pop(
            "gaussian", True
        )  # add gaussian noise in the net latent # True | False
        full_network_G["plus"] = network_G.pop(
            "plus", False
        )  # use the ESRGAN+ modifications # true | false
        full_network_G["finalact"] = network_G.pop(
            "finalact", None
        )  # Activation function, ie use "tanh" to make outputs fit in [-1, 1] range. Default = None. Coordinate with znorm.
        full_network_G["upscale"] = network_G.pop("scale", scale)
        full_network_G["upsample_mode"] = network_G.pop(
            "upsample_mode", "upconv"
        )  # the type of upsample to use
    elif kind_G in ("mrrdb_net", "mesrgan"):
        # ESRGAN modified arch:
        full_network_G[
            "type"
        ] = "mrrdb_net"  # MRRDB_net (modified/"new" arch) | sr_resnet
        full_network_G["in_nc"] = network_G.pop(
            "in_nc", 3
        )  # num. of input image channels: 3 for RGB and 1 for grayscale
        full_network_G["out_nc"] = network_G.pop(
        ...
        full_network_G["alpha"] = network_G.pop(
            "alpha", 1
        )  # interpolation value between percepual and structure
        ...
        full_network_G["unf"] = network_G.pop(
            "unf", 24
        )  # number of filters during upscale
    ...
    # image to image translation
        ...
        full_network_G["input_nc"] = network_G.pop(
            "in_nc", 3
        )  # # of input image channels: 3 for RGB and 1 for grayscale
        full_network_G["output_nc"] = network_G.pop(
            "out_nc", 3
        )  # # of output image channels: 3 for RGB and 1 for grayscale
        if kind_G in ("unet_128", "p2p_128"):
            full_network_G["num_downs"] = network_G.pop(
                "num_downs", 7
            )  # for 'unet_128' (for 128x128 input images)
        elif kind_G in ("unet_256", "p2p_256"):
            full_network_G["num_downs"] = network_G.pop(
                "num_downs", 8
            )  # for 'unet_256' (for 256x256 input images)
        else:
            full_network_G["num_downs"] = network_G.pop(
                "num_downs", 8
            )  # 7 for 'unet_128' (for 128x128 input images) | 8 for 'unet_256' (for 256x256 input images)
        ...
        )  # deconv | upconv # the type of upsample to use, deconvolution or upsample+convolution
        ...
        if kind_G in ("resnet_6blocks", "resnet_6", "cg_6"):
            full_network_G["n_blocks"] = network_G.pop(
                "n_blocks", 6
            )  # 6 for resnet_6blocks (with 6 Resnet blocks) and
        elif kind_G in ("resnet_9blocks", "resnet_9", "cg9"):
            full_network_G["n_blocks"] = network_G.pop(
                "n_blocks", 9
            )  # 9 for resnet_9blocks (with 9 Resnet blocks)
        else:
            full_network_G["n_blocks"] = network_G.pop(
                "n_blocks", 9
            )  # 6 for resnet_6blocks (with 6 Resnet blocks) and 9 for resnet_9blocks (with 9 Resnet blocks)
        full_network_G["ngf"] = network_G.pop(
            "ngf", 64
